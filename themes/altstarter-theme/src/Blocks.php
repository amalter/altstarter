...
class Blocks implements ModuleInterface {
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( ALTSTARTER_THEME_BLOCK_DIST_DIR ) ) {
			$block_json_files = glob( ALTSTARTER_THEME_BLOCK_DIST_DIR . '*/block.json' );
	...
	public function blocks_editor_styles() {
		wp_enqueue_style(
			...
			ALTSTARTER_THEME_TEMPLATE_URL . '/dist/css/editor-style-overrides.css',
			[],
		...
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			wp_enqueue_script(
				'editor-style-overrides',
				ALTSTARTER_THEME_TEMPLATE_URL . '/dist/js/editor-style-overrides.js',
	...
	public function enqueue_block_specific_styles() {
		$stylesheets = glob( ALTSTARTER_THEME_DIST_PATH . 'blocks/autoenqueue/**/*.css' );
		...
		foreach ( $stylesheets as $stylesheet_path ) {
			$block_type = str_replace( ALTSTARTER_THEME_DIST_PATH . 'blocks/autoenqueue/', '', $stylesheet_path );
			$block_type = str_replace( '.css', '', $block_type );
			$asset_file = ALTSTARTER_THEME_DIST_PATH . 'blocks/autoenqueue/' . $block_type . '.asset.php';
			...
			wp_register_style(
				"altstarter-theme-{$block_namespace}-{$block_name}",
				ALTSTARTER_THEME_DIST_URL . 'blocks/autoenqueue/' . $block_type . '.css',
				$asset_file['version'],
			...
			wp_enqueue_block_style(
				$block_type,
				[
					'handle' => "altstarter-theme-{$block_namespace}-{$block_name}",
	...
	public function register_block_pattern_categories() {
		...
		register_block_pattern_category(
			'10up-theme',
			[ 'label' => __( 'altstarter Theme', 'altstarter-theme' ) ]
		);
	}
}
